import kotlin.concurrent.thread // unlocks threads
import java.util.concurrent.locks.ReentrantLock //allows us to use locks and conditions

class Worker(val name: String) : Runnable { // The Runnable interface should be implemented by any class whose instances are intended to be executed by a thread
...
fun main() {
    ...
    val unsyncTasks = List(5) { thread {Worker("Task ${it + 1}").run()}} // no index defined so we use $it
    unsyncTasks.forEach {it.join()} // for each thread .join(). we use $it because no index was defined

    println("======SYNC======")
    val access = ReentrantLock() // instance of ReentrantLock class. provides a mutual exclusion lock allowing threads to lock and unlock
    ...
    val syncTasks = List(5) { i -> // create lambda function. i is used for task order. each iteration of i executes the lambda function. we dont use it since we defined i
        thread() { // creates a thread that will run the specified code
            ...
            finally { // finally garuntees that the lock will be unlocked if something goes wrong
                access.unlock() // need to unlock the lock so the next thread can execute the lambda function. when hold count == 0 it releases the lock
            }
        }
